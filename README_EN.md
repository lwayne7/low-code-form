...
- **Ant Design theming**: enable `darkAlgorithm` via `ConfigProvider` when in dark mode for consistent contrast/readability (`src/App.tsx`)
...
- **Undo/redo patch history**: replace full-tree snapshots with patch entries + structural sharing to cut memory/GC; example (100 adds) serialized history size `~482KB → ~23KB` (~**-95%**) (`src/store.ts`, `src/utils/componentTreeOps.ts`, `src/components/common/HistoryPanel.tsx`)
...
- **Expression type hints (zod)**: add field existence + potential type-mismatch warnings on top of the safe AST, reducing linkage configuration errors (`src/utils/visibleOnTypeValidation.ts`, `src/components/PropertyPanel/LinkageConfig.tsx`)
